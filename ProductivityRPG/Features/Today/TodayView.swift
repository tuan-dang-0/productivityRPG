...
struct TodayView: View {
    ...
    var body: some View {
        NavigationStack {
                        ...
                        ForEach([-1, 0, 1], id: \.self) { weekIndex in
                            HStack(spacing: 4) {
                                ForEach(weekDays(offset: weekIndex), id: \.self) { date in
                                    WeekDayTab(date: date, isSelected: Calendar.current.isDate(date, inSameDayAs: selectedDate), completion: completionForDay(date)) {
                                        selectedDate = date
                                    }
                                }
                            }
                    ...
                    LazyVStack(spacing: 16) {
                        if selectedDayBlocks.isEmpty {
                            ContentUnavailableView(
                                ...
                                description: Text("No blocks for this day")
                            )
                        } else {
            ...
            .overlay {
                if let tutorial = tutorialState, tutorial.isActive, tutorial.currentTutorialStep == .completeBlock {
                    TutorialOverlay(
                        step: .completeBlock,
                        onSkip: {
